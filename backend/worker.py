...
class PlanGenerationWorker:
    ...
    def _generate_plan(self, keyword: str, session_id: str):
        ...
        # Step 2: Create research summary for planner
        research_summary = "\n".join([
            f"- {r['title']}: {r['content']}"
...
class BlogGenerationWorker:
    ...
    def _generate_blog(self, plan: BlogPlan, session_id: str, research_data: Optional[List[Dict[str, Any]]] = None) -> str:
        ...
        MAX_RESEARCH_LENGTH = 131072  # Limit to 131072 characters
        ...
        for r in research_data:
            ...
            if current_length + len(separator) + len(part) > MAX_RESEARCH_LENGTH:
                # Try to add partial content if there's room
                remaining = MAX_RESEARCH_LENGTH - current_length - len(separator)
        ...
        if len(research_content) > MAX_RESEARCH_LENGTH:
            research_content = research_content[:MAX_RESEARCH_LENGTH]
        ...
        logger.info(f"[{session_id}] ✓ Prepared research context ({len(research_content)}/{MAX_RESEARCH_LENGTH} chars, {len(research_parts)} sources)")
